...
export const PlanStep: React.FC<PlanProps> = ({ formData, setFormData }) => {
    const handleTileClick = (event: React.MouseEvent<HTMLButtonElement>) => {
        const currPlan = event.currentTarget.name

        const tempData: string[] = [...formData.plans]
        if (tempData.includes(currPlan)) {
            const index = tempData.indexOf(currPlan);
            tempData.splice(index, 1);
        } else {
            tempData.push(currPlan)
        }
    ...
    return (
        <>
            ...
            <p>You have option of following plans</p>
            <div className="plan_tiles">
                <button className={`tile ${formData.plans.includes('arcade_tile') ? 'tile_selected' : ''}`} onClick={handleTileClick} name="arcade_tile">
                ...
                <button className={`tile ${formData.plans.includes('advanced_tile') ? 'tile_selected' : ''}`} name="advanced_tile" onClick={handleTileClick}>
                ...
                <button className={`tile ${formData.plans.includes('pro_tile') ? 'tile_selected' : ''}`} onClick={handleTileClick} name="pro_tile">
